...
def export_error_table_to_csv(arr_n,filename):
    ...
    for n in arr_n:
        ...
        arr_max_err_newton.append(f"{max_error(x_values, arr_nodes_chebysev, nw.newton_interpolation):.5e}")
        arr_max_err_lagrange.append(f"{max_error(x_values, arr_nodes_chebysev, l.lagrange_interpolation):.5e}")  
        arr_avg_err_newton.append(f"{avg_error(x_values, arr_nodes_chebysev, nw.newton_interpolation):.5e}")
        arr_avg_err_lagrange.append(f"{avg_error(x_values, arr_nodes_chebysev, l.lagrange_interpolation):.5e}")      
    ...
    return
#export_error_table_to_csv([i for i in range(2,36,1)],"./data/chebysev-out-2-35-1")
#export_error_table_to_csv([i for i in range(40,61,5)],"./data/chebysev-out-40-60-5")
export_error_table_to_csv([i for i in range(60,210,10)],"./data/evenly-out-60-200-10")
